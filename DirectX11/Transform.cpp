...
Transform::Transform(const DirectX::XMFLOAT3& scale, const DirectX::XMFLOAT3& rotation, const DirectX::XMFLOAT3& position) : m_Scale(scale), m_Rotation(rotation), m_Position(position) {}
...
void Transform::Translate(const XMFLOAT3& direction, float magnitude)
{
	...
	XMVECTOR newPostion = XMVectorMultiplyAdd(XMVectorReplicate(magnitude), directionVec, XMLoadFloat3(&m_Position));
	XMStoreFloat3(&m_Position, newPostion);
}
...
void Transform::LookAt(const XMFLOAT3& target, const XMFLOAT3& up)
{
	...
	XMFLOAT4X4 rotMatirx;
	XMStoreFloat4x4(&rotMatirx, InvView);
...
XMFLOAT3 Transform::GetEulerAnglesFromRotationMatrix(const XMFLOAT4X4& rotationMatrix)
{
	...
	rotation.x = atan2f(rotationMatrix(2, 0), rotationMatrix(2, 2));
	return rotation;
}
