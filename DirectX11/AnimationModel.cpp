		...
		if (m_Scene->mMaterials[m]->GetTexture(aiTextureType_DIFFUSE, 0, &path) == AI_SUCCESS)
		{
			if (path.data[0] == '*')
			{
				if (m_Texture[path.data] == nullptr)
				{
					...
					D3DX11CreateShaderResourceViewFromMemory(CRenderer::GetDevice().Get(), (const unsigned char*)m_Scene->mTextures[id]->pcData, m_Scene->mTextures[id]->mWidth, nullptr, nullptr, &texture, nullptr);
					assert(texture);
			...
			else
			{
				...
				D3DX11CreateShaderResourceViewFromFileA(CRenderer::GetDevice().Get(),textureName.c_str(),NULL,NULL,&texture,NULL);
...
void CAnimationModel::Calculate(unsigned int m,unsigned int v, aiMesh* mesh, VERTEX_3D* vertex)
{
...
void CAnimationModel::Update(const std::string animationName, int Frame)
{
...
void CAnimationModel::Update(const std::string animationName,const std::string animationName2, float blendRate, int Frame)
{
	//if (m_CurAnimationName != animationName)
	//{
	//	m_t -= 0.1f;
...
void CAnimationModel::Draw()
{
	...
	for (unsigned int m = 0; m < m_Scene->mNumMeshes; m++)
	{
		...
		// ポリゴン描画
		CRenderer::GetDeviceContext()->DrawIndexed(m_Scene->mMeshes[m]->mNumFaces * 3, 0, 0);
	}
}

void CAnimationModel::LoadAnimation(const std::string filename, const std::string animationName)
{
